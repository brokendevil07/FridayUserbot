...
CUSTOM_STICKER_NAME = (str(CUSTOM_STICKER_PACK_NAME)
                       if CUSTOM_STICKER_PACK_NAME else
                       "My Boss DEVIL Volume Pack One")
CUSTOM_ANIME_PACK = (str(CUSTOM_ANIMATED_PACK_NAME)
                     if CUSTOM_ANIMATED_PACK_NAME else "My Boss Animated Pack")
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    if not await stickerset_exists(bot_conv, packshortname):
                        await event.edit("**Pack No. **" + str(prevv) +
                                         "** SABAR KRO , Vol. **" + str(pack))
                        if is_a_s:
                ...
                await silently_send_message(bot_conv, "/done")
    await event.edit(
        f"**STICKER BAN GYA UTHA LO** [YHA SE UTHAO CB](t.me/addstickers/{packshortname})"
    )
